...
export default function MenuMobileNav({ categories, openCategoryAccordions, toggleCategoryAccordion }) {
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories
      ...
      .filter(
        (category) => category.name.toLowerCase().includes(searchQuery.toLowerCase()) || category.items.length > 0
      )
        ...
        >
          ...
          >
            {filteredCategories.map((category, index) => (
              <AccordionItem key={`category-${category.id}-${index}`} value={`category-${index}`}>

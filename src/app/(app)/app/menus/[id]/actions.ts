...
import { z } from "zod"
import db from "@/db"
import { MenuSchema, TogglePublishMenuSchema, categories, menuItems, menus } from "@/db/schema/menus"
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      for (const inputCategory of inputCategories) {
        const { id: inputCategoryId, menuItems: inputMenuItems, ...categoryUpdateData } = inputCategory
        ...
        const menuItemsToRemove = [...existingMenuItemIds].filter((id) => !updatedMenuItemIds.has(id))
        if (menuItemsToRemove.length > 0) {
          await tx
            .delete(menuItems)
            .where(and(eq(menuItems.categoryId, categoryId), inArray(menuItems.id, menuItemsToRemove)))
        }
      }

      // Remove categories that are no longer present
      const categoriesToRemove = [...existingCategoryIds].filter((id) => !updatedCategoryIds.has(id))
      if (categoriesToRemove.length > 0) {
        await tx.delete(categories).where(and(eq(categories.menuId, id), inArray(categories.id, categoriesToRemove)))
      }
    })
...
function hasChanged(existing: Record<string, unknown>, updated: Record<string, unknown>): boolean {
  return Object.keys(updated).some((key) => existing[key] !== updated[key])
}

export async function toggleMenuPublicStatus(data: z.infer<typeof TogglePublishMenuSchema>) {
  const user = await requireAuth()

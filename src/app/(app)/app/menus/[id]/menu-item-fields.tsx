...
export function MenuItemsFields({ form, categoryIndex }: { form: unknown; categoryIndex: number }) {
  ...
  return (
    <>
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">These items will show within the category.</p>
      </div>
          ...
          >
            <AccordionTrigger className="text-left hover:no-underline">
              <div className="flex items-center gap-2">
                {form.watch(`categories.${categoryIndex}.menuItems.${itemIndex}.posId`) && (
                  <p className="text-xl">{form.watch(`categories.${categoryIndex}.menuItems.${itemIndex}.posId`)}</p>
                )}
                <h6 className="text-xl font-medium">
                  {form.watch(`categories.${categoryIndex}.menuItems.${itemIndex}.name`) || `Item ${itemIndex + 1}`}
                </h6>
                {form.watch(`categories.${categoryIndex}.menuItems.${itemIndex}.isHidden`) && (
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>An optional custom ID commonly used to match with a POS system.</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <MultipleSelector
                          value={field.value.map(
                            (tag: string) =>
                              menuItemTags.find((item) => item.value === tag) || { value: tag, label: tag }
                          )}
                          onChange={(newValue) => {
                            const tagValues = newValue.map((item) => item.value)
                      ...
                      <FormDescription>Select tags that apply to this menu item.</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription>Hide this item from the menu if it is not available.</FormDescription>
                      </div>
                      <FormControl>

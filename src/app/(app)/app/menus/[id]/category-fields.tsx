...
export function CategoryFields({ form, toggleCategoryAccordion, openCategoryAccordions }) {
  const {
  ...
  return (
    <>
      <CardHeader>
        ...
        <CardDescription>These are the categories that will show on the menu.</CardDescription>
      </CardHeader>
      <CardContent>
        <Accordion
          type="multiple"
                ...
                <AccordionTrigger className="px-4 py-2 hover:no-underline">
                  <div className="flex items-center gap-2">
                    <h5 className="text-2xl font-medium">
                      {form.watch(`categories.${categoryIndex}.name`) || `Category ${categoryIndex + 1}`}
                    </h5>
                ...
                <AccordionContent className="border-t">
                  <div className="relative flex flex-col gap-6 space-y-4 p-4">
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-row items-center justify-between">
                          <div className="space-y-0.5">
                            ...
                            <FormDescription>
                              Hide this category & all it's items from the menu if it is not available.
                            </FormDescription>
                          </div>
                          <FormControl>
                        ...
                        render={() => (
                          <FormItem>
                            ...
                            <div className="space-y-2">
                              {form.watch(`categories.${categoryIndex}.notes`, []).map((note, noteIndex) => (
                                <div key={noteIndex} className="flex items-center space-x-2">
                                  <Asterisk className="h-4 w-4" />
                                  <FormControl>
                                    <Input
                                      {...form.register(`categories.${categoryIndex}.notes.${noteIndex}`)}
                                  ...
                                  <Button
                                    ...
                                    size="icon"
                                    onClick={() => {
                                      const currentNotes = form.getValues(`categories.${categoryIndex}.notes`)
                                      form.setValue(
                                        `categories.${categoryIndex}.notes`,
                                        currentNotes.filter((_, index) => index !== noteIndex)
                                      )
                                    }}
                                  >
                                    <X className="h-4 w-4" />
                            ...
                            <FormDescription>Additional information or notes about this category.</FormDescription>
                            <FormMessage />
                          </FormItem>
                        )}
                      />

                      {form.watch(`categories.${categoryIndex}.notes`, []).length <= 3 && (
                        <Button
                          type="button"
                          size="sm"
                          ...
                          onClick={() => {
                            const currentNotes = form.getValues(`categories.${categoryIndex}.notes`) || []
                            form.setValue(`categories.${categoryIndex}.notes`, [...currentNotes, ""])
                          }}
                        >
                    ...
                    <div className="space-y-4">
                      <MenuItemsFields form={form} categoryIndex={categoryIndex} />
                    </div>
                  </div>
                </AccordionContent>
              </AccordionItem>

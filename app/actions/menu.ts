...
import { menuTable, categoryTable, menuItemTable } from "@/db/schema"
...
export const getMenuWithDetails = async (menu_id: number) => {
  ...
  const categories = await db.select().from(categoryTable).where(eq(categoryTable.menu_id, menu_id))

  // Fetch the menu items for each category
  for (const category of categories) {
    const menuItems = await db.select().from(menuItemTable).where(eq(menuItemTable.category_id, category.id))
...
// Add a new menu
export const addMenu = async (title: string, description: string, logo_src?: string, image_src?: string) => {
...
export const getCategories = async (menu_id: number) => {
  const data = await db.select().from(categoryTable).where(eq(categoryTable.menu_id, menu_id))
  return data
}
...
// Edit a category
export const editCategory = async (id: number, title: string, description: string, image_src?: string) => {
  await db
...
export const getMenuItems = async (category_id: number) => {
  const data = await db.select().from(menuItemTable).where(eq(menuItemTable.category_id, category_id))
  return data
}

...
import { addMenu, getMenus } from "@/app/actions/menu"
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 max-w-3xl mx-auto p-6">
        ...
        <div className="space-y-6">
          ...
          {categoryFields.map((categoryField, categoryIndex) => (
            <div key={categoryField.id} className="border rounded-lg overflow-hidden">
              <Accordion type="single" collapsible className="w-full">
                <AccordionItem value="category" className="border-none">
                  <AccordionTrigger className="px-4 py-2 hover:no-underline">
                    {form.watch(`categories.${categoryIndex}.title`) || `Category ${categoryIndex + 1}`}
                  </AccordionTrigger>
  ...
  return (
    <Accordion type="multiple" className="space-y-4">
      {fields.map((field, itemIndex) => (
        <AccordionItem key={field.id} value={`item-${categoryIndex}-${itemIndex}`} noBorder>
          <AccordionTrigger className="text-left hover:no-underline">
            {form.watch(`categories.${categoryIndex}.items.${itemIndex}.title`) || `Item ${itemIndex + 1}`}
          </AccordionTrigger>
          <AccordionContent>
